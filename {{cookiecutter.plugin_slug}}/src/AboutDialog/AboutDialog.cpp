...
INT_PTR CALLBACK abtDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch(uMsg) {
		case WM_INITDIALOG:
			...
			Edit_SetText(GetDlgItem(hwndDlg, IDC_VERSION), L"{{ cookiecutter.plugin_slug }} v" VERSION_TEXT L" " BITNESS);
			return true;
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				case IDOK:
					...
					return true;
				case IDC_GITHUB:
					ShellExecute(hwndDlg, L"open", L"https://github.com/xxx/{{ cookiecutter.plugin_slug }}/", NULL, NULL, SW_SHOWNORMAL);
					return true;
			}
		case WM_DESTROY:
			DestroyWindow(hwndDlg);
